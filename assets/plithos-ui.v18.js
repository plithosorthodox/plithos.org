 ...
 *      fetches data/search-index.v11.<lang>.json, once, on first open,
 *      and again if the reader changes language.
 *
 *      There is one index per language and each carries the names as that
 *      language prints them, with the English name kept beside any that
 *      differs. So a Greek reader is answered in Greek and can still find
 *      a saint by the English name he happens to know.
 *
...
(function () {
  ...
  var INDEX_STEM = "data/search-index.v11.";
  ...
  /* Everything the shared chrome says to a reader is held here rather than
     ...
     data/ui-i18n.v6.en.json is the same table as a file: it is the sheet a
     translator copies. The other languages are read from
     data/ui-i18n.v6.<code>.json, fetched once beside the index.
  ...
  function loadWords() {
    ...
    langWords = fetch("data/ui-i18n.v6." + lang + ".json")
      .then(function (r) {
  ...
  var DATA = null, loading = null, loadedLang = null;

  function fetchIndex(lang) {
    return fetch(INDEX_STEM + lang + ".json")
      .then(function (r) {
  ...
  function loadIndex() {
    var lang = pageLang();
    if (DATA && loadedLang === lang) return Promise.resolve(DATA);
    if (loading && loadedLang === lang) return loading;
    loadedLang = lang;
    /* A language with no index of its own is answered by the English one
       rather than by nothing: the reader sees English names, which is what
       he saw before there were any others. */
    loading = fetchIndex(lang)
      .then(function (d) {
        return (d && d.e) ? d : (lang === "en" ? null : fetchIndex("en"));
      })
      .then(function (d) {
  ...
  function score(entry, q) {
    ...
    /* Every name that has been translated keeps its English beside it, and
       both are searched. A reader who knows a saint as Nicholas finds him in
       the Greek index and is shown Νικόλαος. */
    if (entry.e) {
      s = Math.max(s, rank(entry._e || (entry._e = fold(entry.e)), q));
    }
  ...
  function onLangChange() {
    langWords = null;
    T = EN;
    /* The index is the reader's language too, so it goes with the words.
       Dropping it here rather than translating in place also clears the
       folded forms cached on every entry. */
    DATA = null;
    loading = null;
    loadedLang = null;
