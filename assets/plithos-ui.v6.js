 ...
 *      at once - saints, prayers, library works, scripture books, glossary
 *      terms, and the tags the Library shelf is sorted by. Each page holds
 *      only its own dataset inline and cannot see the others', so the palette
 *      fetches data/search-index.v4.json. It is fetched once, on first open,
 *      so it costs nothing on pages nobody searches from.
 *
...
(function () {
  ...
  var INDEX_URL = "data/search-index.v4.json";
  ...
  /* Everything the shared chrome says to a reader is held here rather than
     ...
     data/ui-i18n.v1.en.json is the same table as a file: it is the sheet a
     translator copies. The other languages are read from
     data/ui-i18n.v1.<code>.json, fetched once beside the index.
  ...
  function useWords(d) {
    ...
    for (k in d) if (d.hasOwnProperty(k)) {
      if (d[k] && typeof d[k] === "object" && !(d[k] instanceof Array) && EN[k]) {
        ...
        for (j in d[k]) if (d[k].hasOwnProperty(j)) sub[j] = d[k][j];
        out[k] = sub;
      } else if (d[k]) out[k] = d[k];
    }
    T = out;
  }
  ...
  function loadWords() {
    ...
    langWords = fetch("data/ui-i18n.v1." + lang + ".json")
      .then(function (r) {
