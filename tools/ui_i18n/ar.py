# -*- coding: utf-8 -*-
"""AR interface strings. TEXT = {"surface|key": the rendering}.

Written against tools/loop_ui.py, which derives what is missing from the
pages themselves. Installed by tools/build_ui_i18n.py; nothing here edits a
page directly, so two languages can be written at once.
"""
TEXT = {

"index.I18N|ui.about":
"عن الموقع",

"index.I18N|ui.guide":
"دليل",

"prayers.T|all":
"كل الصلوات",

"prayers.T|back":
"كل الصلوات",

"prayers.T|copied":
"تم النسخ",

"prayers.T|copy":
"نسخ",

"prayers.T|h1":
"الصلوات",

"prayers.T|lede":
"صلوات الكنيسة الأرثوذكسية التقليدية، مرتبة كأنها كتاب صلوات. اختر قسما أو ابحث.",

"prayers.T|n_many":
"# صلوات",

"prayers.T|n_one":
"صلاة واحدة",

"prayers.T|navCal":
"التقويم",

"prayers.T|navContact":
"اتصل بنا",

"prayers.T|navGlossary":
"مسرد",

"prayers.T|navLib":
"المكتبة",

"prayers.T|navPrayers":
"الصلوات",

"prayers.T|navRule":
"القانون",

"prayers.T|navSaints":
"القديسون",

"prayers.T|none":
"لا صلاة تطابق ذلك.",

"prayers.T|partial":
"هذه الصلاة غير متوفرة بعد بهذه اللغة، وهي معروضة بالإنكليزية.",

"prayers.T|ph":
"ابحث في الصلوات",

"prayers.T|sections":
"الأقسام",

"prayers.T|src":
"المصدر",

"prayers.T|words":
"كلمات",

"names|Apostle Epίmakhos of Alexandria":
"الشهيد إبيماخوس الإسكندري",

"names|Appearance of the Icon of the Mother of God “The Footprint” at Pochaiv":
"ظهور أيقونة والدة الإله «أثر القدم» في بوتشاييف",

"names|Arrival of the Ivḗron Icon of the Mother of God in Georgia":
"وصول أيقونة والدة الإله الإيفيرية إلى جورجيا",

"names|Ascension Day":
"صعود الرب",

"names|Ash Wednesday — Lent begins":
"أربعاء الرماد - بدء الصوم",

"names|Blessed John “the Hairy” and Fool-For-Christ at Rostov":
"الطوباوي يوحنا «الأشعر»، المتباله من أجل المسيح الروستوفي",

"names|Clean Monday — Great Lent begins":
"الاثنين النقي - بدء الصوم الكبير",

"names|Corpus Christi":
"عيد جسد الرب",

"names|EASTER DAY — The Resurrection of the Lord":
"أحد الفصح - قيامة الرب",

"names|Entry of the Lord into Jerusalem":
"دخول الرب إلى أورشليم (الشعانين)",

"names|Finding of the relics of Righteous Saint Νikόdēmos":
"اكتشاف رفات البار نيقوديموس",

"names|First Sunday in Lent":
"الأحد الأول من الصوم",

"names|Forgiveness Sunday (Cheesefare)":
"أحد المغفرة (مرفع الجبن)",

"names|Fourth Sunday in Lent (Laetare)":
"الأحد الرابع من الصوم (Laetare)",

"names|Good Friday":
"الجمعة العظيمة",

"names|Great and Holy Friday":
"الجمعة العظيمة المقدسة",

"names|Great and Holy Monday":
"الاثنين العظيم المقدس",

"names|Great and Holy Saturday":
"السبت العظيم المقدس",

"names|Great and Holy Thursday":
"الخميس العظيم المقدس",

"names|Great and Holy Tuesday":
"الثلاثاء العظيم المقدس",

"names|Great and Holy Wednesday":
"الأربعاء العظيم المقدس",

"names|Greatmartyr Eustáthios Placidas, with his wife and children, of Rome":
"الشهيد العظيم إفستاثيوس بلاكيداس، مع زوجته وأولاده، الروماني",

"names|Greatmartyr Theodore Stratelates “the General”":
"الشهيد العظيم ثيودوروس قائد الجيش",

"names|Hieromartyr Alexander of Sίdē, in Pamphylia":
"الشهيد في الكهنة ألكسندروس السيدي، في بمفيلية",

"names|Hieromartyr Mίlos (or Milēs) the Wonderworker, and two disciples":
"الشهيد في الكهنة ميلوس العجائبي وتلميذاه",

"names|Hieromartyr Nikḗtas":
"الشهيد في الكهنة نيكيتاس",

"names|Hieromartyr Zenobios and his sister Zenobia, of Aegæ in Cilicia":
"الشهيد في الكهنة زينوبيوس وأخته زينوبيا، من إيجة كيليكية",

"names|Holy Empress Markianḗ":
"القديسة الإمبراطورة مركياني",

"names|Holy Saturday":
"السبت العظيم",

"names|Holy Virgin Martyr Theodosίa of Tyre":
"الشهيدة العذراء القديسة ثيودوسيا الصورية",

"names|Icon of the Mother of God of Mount Athos, “Sweet Kissing”":
"أيقونة والدة الإله «القبلة الحلوة» (غليكوفيلوسا) الآثوسية",

"names|Icon of the Mother of God of Mt. Athos, “Sweet Kissing”":
"أيقونة والدة الإله «القبلة الحلوة» (غليكوفيلوسا) الآثوسية",

"names|Icon of the Mother of God “The Unbreakable Wall”":
"أيقونة والدة الإله «السور المنيع»",

"names|Icon of the Mother of God “of the Passion”":
"أيقونة والدة الإله «الآلام»",

"names|Icon of the Mother of God “of the Sign”, the “Kursk-Root”":
"أيقونة والدة الإله «الآية»، «جذر كورسك»",

"names|Icon of the Mother of God “the Joy of All who Sorrow” (with coins) in St. Petersburg":
"أيقونة والدة الإله «فرح كل المحزونين» (مع القطع النقدية) في سانت بطرسبرغ",

"names|Icon of the Mother of God “the Surety of Sinners”":
"أيقونة والدة الإله «ضامنة الخطأة»",

"names|Lazarus Saturday":
"سبت لعازر",

"names|Martyr Ariádnē of Phrygia":
"الشهيدة أريادني الفريجية",

"names|Martyr Boniface at Tarsus in Cilicia, and Righteous Aglaϊa of Rome":
"الشهيد بونيفاتيوس، في طرسوس كيليكية، والبارة أغلاييا الرومانية",

"names|Martyr Claudius, Asterius, Neon, and Theonilla of Aegæ in Cilicia":
"الشهداء كلافديوس وأستيريوس ونيون وثيونيلا من إيجة كيليكية",

"names|Martyr Drosίs, daughter of Emperor Trajan":
"الشهيدة دروسيس، ابنة الإمبراطور تراجان",

"names|Martyr Dēmḗtrios of Thrace":
"الشهيد ديميتريوس التراقي",

"names|Martyr Manetha of Cæsarea in Palestine":
"الشهيدة مانيثا القيصرية، في فلسطين",

"names|Martyr Savva Stratelates “the General” of Rome, and 70 soldiers with him":
"الشهيد سابا قائد الجيش الروماني و70 جنديا معه",

"names|Martyr Tryphaίnē at Cyzicus":
"الشهيدة تريفيني في كيزيكوس",

"names|Martyrs Andrew, John, and John’s children: Peter and Antonius, of Syracuse, martyred in Africa":
"الشهداء أندراوس ويوحنا وابنا يوحنا بطرس وأنطونينوس، السيراكوسيون، الذين استشهدوا في أفريقيا",

"names|Martyrs Paul and two sisters, Chionia (Thea) and Alevtina (Valentina), at Cæsarea in Palestine":
"الشهداء بولس وأختان، خيونيا (ثيا) وأليفتينا (فالنتينا)، في قيصرية فلسطين",

"names|Martyrs Rhipsime and Gaianḗ of Armenia and those with them":
"الشهيدتان ريبسيميا وغاياني الأرمنيتان ومن معهما",

"names|Martyrs Victorinus, Victor, Nikēphóros, Claudius, Diodorus, Serapion, and Papias, of Egypt":
"الشهداء فيكتورينوس وفيكتور ونيكيفوروس وكلافديوس وديودوروس وسيرابيون وبابياس، المصريون",

"names|Maundy Thursday":
"الخميس العظيم",

"names|PASCHA — The Resurrection of Christ":
"الفصح - قيامة المسيح",

"names|Palm Sunday":
"أحد الشعانين",

"names|Passion Sunday":
"أحد الآلام",

"names|Pentecost — The Holy Trinity":
"العنصرة - الثالوث الأقدس",

"names|Procession of the Honorable Wood of the Life-Giving Cross of the Lord (First of the three “Feasts of the Savior” in August)":
"خروج خشبة الصليب الكريم المحيي",

"names|Quinquagesima Sunday":
"أحد الخمسينية قبل الفصح",

"names|Righteous Abel the Shepherd":
"البار هابيل الراعي",

"names|Righteous Forefather Adam":
"البار الجد آدم",

"names|Righteous Forefather Arphaxad":
"البار الجد أرفكشاد",

"names|Righteous Forefather Cainan":
"البار الجد قينان",

"names|Righteous Forefather Eber":
"البار الجد عابر",

"names|Righteous Forefather Enoch":
"البار الجد أخنوخ",

"names|Righteous Forefather Enos":
"البار الجد أنوش",

"names|Righteous Forefather Isaac":
"البار الجد إسحق",

"names|Righteous Forefather Jacob":
"البار الجد يعقوب",

"names|Righteous Forefather Jared":
"البار الجد يارد",

"names|Righteous Forefather Lamech":
"البار الجد لامك",

"names|Righteous Forefather Mahalalel":
"البار الجد مهللئيل",

"names|Righteous Forefather Methuselah":
"البار الجد متوشالح",

"names|Righteous Forefather Nahor":
"البار الجد ناحور",

"names|Righteous Forefather Noah":
"البار الجد نوح",

"names|Righteous Forefather Peleg":
"البار الجد فالج",

"names|Righteous Forefather Reu":
"البار الجد رعو",

"names|Righteous Forefather Salah":
"البار الجد شالح",

"names|Righteous Forefather Serug":
"البار الجد سروج",

"names|Righteous Forefather Seth":
"البار الجد شيث",

"names|Righteous Forefather Terah":
"البار الجد تارح",

"names|Righteous Foremother Bathsheba":
"البارة الجدة بثشبع",

"names|Righteous Foremother Esther":
"البارة الجدة أستير",

"names|Righteous Foremother Eve":
"البارة الجدة حواء",

"names|Righteous Foremother Huldah the Prophetess":
"البارة الجدة خلدة النبية",

"names|Righteous Foremother Judith":
"البارة الجدة يهوديت",

"names|Righteous Foremother Leah":
"البارة الجدة ليئة",

"names|Righteous Foremother Rachel":
"البارة الجدة راحيل",

"names|Righteous Foremother Rebecca":
"البارة الجدة رفقة",

"names|Righteous Foremother Ruth":
"البارة الجدة راعوث",

"names|Righteous Foremother Sarah":
"البارة الجدة سارة",

"names|Righteous Foremother Tamar":
"البارة الجدة ثامار",

"names|Righteous Jael":
"البارة ياعيل",

"names|Righteous Japheth, son of Noah":
"البار يافث ابن نوح",

"names|Righteous Joseph the All-Comely":
"البار يوسف الكلي الجمال",

"names|Righteous Joshua the Son of Nun":
"البار يشوع بن نون",

"names|Righteous Lot":
"البار لوط",

"names|Righteous Melchizedek, King of Salem":
"البار ملكيصادق ملك ساليم",

"names|Righteous Miriam, sister of Moses":
"البارة مريم أخت موسى",

"names|Righteous Patriarch Asher":
"البار البطريرك آشير",

"names|Righteous Patriarch Dan":
"البار البطريرك دان",

"names|Righteous Patriarch Gad":
"البار البطريرك جاد",

"names|Righteous Patriarch Issachar":
"البار البطريرك يساكر",

"names|Righteous Patriarch Judah":
"البار البطريرك يهوذا",

"names|Righteous Patriarch Levi":
"البار البطريرك لاوي",

"names|Righteous Patriarch Naphtali":
"البار البطريرك نفتالي",

"names|Righteous Patriarch Reuben":
"البار البطريرك رأوبين",

"names|Righteous Patriarch Simeon":
"البار البطريرك شمعون",

"names|Righteous Patriarch Zebulun":
"البار البطريرك زبولون",

"names|Righteous Shem, son of Noah":
"البار سام ابن نوح",

"names|Righteous Solomon the King":
"البار سليمان الملك",

"names|Righteous Virgin Glykerίa of Novgorod":
"البارة العذراء غليكيريا النوفغورودية",

"names|Rogation Sunday":
"أحد الطلبة",

"names|Saint Akakios, Bishop of Melitēnḗ":
"القديس أكاكيوس، أسقف مليتيني",

"names|Saint Clement the Stylite who lived as an ascetic on Mount Ságmata in Boeotia":
"القديس كليمنضس العمودي الذي نسك على جبل ساغماتا في بيوتية",

"names|Saint Emmeleίa":
"القديسة إميليا",

"names|Saint Euphrosynē, Great Princess of Moscow":
"القديسة إفروسيني، الأميرة الكبرى الموسكوفية",

"names|Saint Hosius the Confessor, Bishop of Córdoba":
"القديس هوسيوس المعترف، أسقف قرطبة",

"names|Saint Innocent of Komél and Vologda disciple of Saint Nilus of Sora":
"القديس إينوكنتيوس الكوميلي والفولوغدي، تلميذ القديس نيلوس السورا",

"names|Saint Luke (Voino-Yasenetsky), Archbishop of Simferopol and Crimea (1961)":
"القديس لوقا (فوينو-ياسينيتسكي)، رئيس أساقفة سيمفيروبول والقرم (1961)",

"names|Saint Nektarios, Metropolitan of Pentapolis, Wonderworker of Aegina (1920)":
"القديس نكتاريوس، مطران المدن الخمس، عجائبي إيجينا (1920)",

"names|Saint Nikḗtas, Bishop of Novgorod, Recluse of the Kyiv Far Caves":
"القديس نيكيتاس، أسقف نوفغورود، الحبيس من مغاور كييف البعيدة",

"names|Saint Papias, Bishop of Hierapolis":
"القديس بابياس أسقف هيرابوليس",

"names|Saint Quiricus (Kḗrykos) and his mother Julitta":
"القديسان كيريكوس وأمه يوليطة",

"names|Saint Stephen the “New Light” of Constantinople":
"القديس ستيفانوس «النور الجديد» القسطنطيني",

"names|Saint Theophanó the Wonderworker, wife of Emperor Leo the Wise":
"القديسة ثيوفانو العجائبية، زوجة الإمبراطور لاون الحكيم",

"names|Saint Theophilus, Bishop of Antioch":
"القديس ثيوفيلوس أسقف أنطاكية",

"names|Second Sunday in Lent":
"الأحد الثاني من الصوم",

"names|Septuagesima Sunday":
"أحد السبعينية قبل الفصح",

"names|Sexagesima Sunday":
"أحد الستينية قبل الفصح",

"names|Sunday of All Saints":
"أحد جميع القديسين",

"names|Sunday of Orthodoxy":
"أحد الأرثوذكسية",

"names|Sunday of St Gregory Palamas":
"أحد القديس غريغوريوس بالاماس",

"names|Sunday of St John Climacus":
"أحد القديس يوحنا السلمي",

"names|Sunday of St Mary of Egypt":
"أحد القديسة مريم المصرية",

"names|Sunday of Zacchaeus":
"أحد زكا العشار",

"names|Sunday of the Blind Man":
"أحد الأعمى",

"names|Sunday of the Fathers of the First Council":
"أحد آباء المجمع المسكوني الأول",

"names|Sunday of the Last Judgment (Meatfare)":
"أحد الدينونة (مرفع اللحم)",

"names|Sunday of the Myrrh-bearing Women":
"أحد حاملات الطيب",

"names|Sunday of the Paralytic":
"أحد المخلع",

"names|Sunday of the Prodigal Son":
"أحد الابن الشاطر",

"names|Sunday of the Publican and the Pharisee":
"أحد الفريسي والعشار",

"names|Sunday of the Samaritan Woman":
"أحد السامرية",

"names|Sunday of the Veneration of the Cross":
"أحد السجود للصليب",

"names|The Ascension of the Lord":
"صعود الرب",

"names|The Holy Transfiguration of our Lord God and Savior Jesus Christ (the Second “Feast of the Savior” in August)":
"تجلي ربنا وإلهنا ومخلصنا يسوع المسيح المقدس",

"names|Third Sunday in Lent":
"الأحد الثالث من الصوم",

"names|Thomas Sunday (Antipascha)":
"أحد توما (الأحد الجديد)",

"names|Translation of the Image “Not-Made-By-Hands” of our Lord Jesus Christ from Edessa to Constantinople, the Third “Feast of the Savior in August”":
"نقل أيقونة ربنا يسوع المسيح غير المصنوعة بيد من الرها إلى القسطنطينية",

"names|Trinity Sunday":
"أحد الثالوث القدوس",

"names|Uncovering of the relics of Saint Nikḗtas, Bishop of Novgorod":
"اكتشاف رفات القديس نيكيتاس، أسقف نوفغورود",

"names|Uncovering of the relics of Saint Vsévolod (Gabriel) of Pskov":
"اكتشاف رفات القديس فسيفولود (غابرييل) البسكوفي",

"names|Venerable Athanasius “the Resurrected One”, Recluse of the Kyiv Near Caves":
"القديس أثناسيوس «القائم من الموت»، الحبيس من مغاور كييف القريبة",

"names|Venerable Dionysius, Archimandrite of Saint Sergius’ Monastery":
"القديس ديونيسيوس، رئيس دير القديس سرجيوس",

"names|Venerable Euphrosynē of Suzdal":
"القديسة إفروسيني السوزدالية",

"names|Venerable Isaac, Founder of Dalmátos Monastery at Constantinople":
"القديس إسحاق، مؤسس دير الدلماتيين في القسطنطينية",

"names|Venerable John Calabytes “the Hut-Dweller”":
"القديس يوحنا الكوخي",

"names|Venerable John Climacus of Sinai, Author of “the Ladder”":
"القديس يوحنا السلمي السينائي، مؤلف «السلم»",

"names|Venerable Nίkander Wonderworker of Pskov":
"القديس نيكاندر العجائبي البسكوفي",

"names|Venerable Paisios the Athonite (1994)":
"البار باييسيوس الآثوسي (1994)",

"names|Venerable Paphnutius and Euphrosynē of Alexandria":
"القديسان بفنوتيوس وإفروسيني الإسكندريان",

"names|Venerable Porphyrios of Kavsokalyvia (1991)":
"البار بورفيريوس الكافسوكاليفي (1991)",

"names|Venerable Romanus the Melodist, “Sweet-Singer”":
"القديس رومانوس المرنم",

"names|Venerable Silouan the Athonite (1938)":
"البار سلوان الآثوسي (1938)",

"names|Venerable Sophrony (Sakharov) of Essex (1993)":
"البار صفرونيوس (ساخاروف) الإسكسي (1993)",

"names|Venerable Theodore Graptus, “the branded,” Confessor":
"القديس ثيودوروس المخطوط «الموسوم»، المعترف",

"names|Venerable Theodore Trichinas “the Hair-Shirt Wearer” and Hermit Near Constantinople":
"القديس ثيودوروس تريخيناس، الناسك قرب القسطنطينية",

"names|Venerable Theoktίstē of the Isle of Lesbos":
"القديسة ثيوكتيستي من جزيرة لسبوس",

"names|Whitsunday (Pentecost)":
"أحد العنصرة",

"names|“Bogolyubov” Icon of the Mother of God":
"أيقونة والدة الإله البوغوليوبية",

"names|“Chukhloma” Icon of the Mother of God from Galich":
"أيقونة والدة الإله «تشوخلوما» من غاليتش",

"names|“Enlightener of Minds” Icon of the Mother of God":
"أيقونة والدة الإله «منيرة العقول»",

"names|“Kozel'shchansk” Icon of the Mother of God":
"أيقونة والدة الإله «كوزلشانسك»",

"names|“O All-Hymned Mother” Icon of the Mother of God":
"أيقونة والدة الإله «يا أيتها الأم الكلية التسبيح»",

"names|“Our Lady of Sitka” Icon of the Mother of God":
"أيقونة والدة الإله «سيدة سيتكا»",

"names|“Ozerianka” Icon of the Mother of God of Shuiu-Smolensk":
"أيقونة والدة الإله «أوزيريانكا» الشويسكية السمولينسكية",

"names|“Seven Arrows” Icon of the Mother of God":
"أيقونة والدة الإله «السهام السبعة»",

"names|“Surety of Sinners” Icon of the Mother of God of Korets":
"أيقونة والدة الإله «ضامنة الخطأة» الكوريتسية",

"names|“Svena” Icon of the Mother of God":
"أيقونة والدة الإله «سفينسك»",

"names|“Virgin of Tenderness” Icon of the Mother of God of the Pskov Caves":
"أيقونة والدة الإله «الحنونة» من مغاور بسكوف",

"library.RLEX|cntLanguages":
"%1 لغات",

"library.RLEX|cntLiturgies":
"%1 قداسات",

"library.RLEX|cntLiturgy":
"%1 قداس",

"library.RLEX|cntSeptuagint":
"السبعينية · بلغات كثيرة",

"library.RLEX|cntTitle":
"%1 عنوان",

"library.RLEX|cntTitles":
"%1 عناوين",

"library.RLEX|cntWork":
"%1 نص",

"library.RLEX|cntWorks":
"%1 نصوص",

"library.RLEX|homeSub":
"مكتبة الآباء والكتب المقدسة والمجامع والقداس الإلهي في الكنيسة الأرثوذكسية. اختر مجموعة لتبدأ.",

"library.RLEX|lang:cu":
"سلافونية كنسية",

"library.RLEX|lx:1st century":
"القرن الأول",

"library.RLEX|lx:2nd century":
"القرن الثاني",

"library.RLEX|lx:3rd century":
"القرن الثالث",

"library.RLEX|lx:4th century":
"القرن الرابع",

"library.RLEX|lx:5th century":
"القرن الخامس",

"library.RLEX|lx:6th century":
"القرن السادس",

"library.RLEX|lx:8th century":
"القرن الثامن",

"library.RLEX|lx:Almsgiving and the poor":
"الصدقة والفقراء",

"library.RLEX|lx:An unknown preacher":
"واعظ مجهول",

"library.RLEX|lx:Angels and demons":
"الملائكة والشياطين",

"library.RLEX|lx:Anonymous, of the sixth century":
"مجهول من القرن السادس",

"library.RLEX|lx:Answering heresy":
"الرد على البدع",

"library.RLEX|lx:Answering the pagans":
"الرد على الوثنيين",

"library.RLEX|lx:Aphrahat the Persian Sage":
"أفراهاط الحكيم الفارسي",

"library.RLEX|lx:Apostolic tradition":
"التقليد الرسولي",

"library.RLEX|lx:Baptism and chrismation":
"المعمودية والميرون",

"library.RLEX|lx:Bishops, priests and deacons":
"الأساقفة والكهنة والشمامسة",

"library.RLEX|lx:Canon law and the Councils":
"القوانين الكنسية والمجامع",

"library.RLEX|lx:Christ foretold in the prophets":
"المسيح المنبأ به في الأنبياء",

"library.RLEX|lx:Christ, God and man":
"المسيح، إله وإنسان",

"library.RLEX|lx:Clement of Alexandria":
"إكليمنضس الإسكندري",

"library.RLEX|lx:Creation":
"الخليقة",

"library.RLEX|lx:Defence of the faith":
"الدفاع عن الإيمان",

"library.RLEX|lx:Deification":
"التأله",

"library.RLEX|lx:Dionysius the Areopagite":
"ديونيسيوس الأريوباغي",

"library.RLEX|lx:Doctrine":
"التعليم العقائدي",

"library.RLEX|lx:Eusebius of Caesarea":
"إفسافيوس القيصري",

"library.RLEX|lx:Fasting":
"الصوم",

"library.RLEX|lx:Free will and providence":
"حرية الإرادة والعناية الإلهية",

"library.RLEX|lx:Hymns":
"التسابيح",

"library.RLEX|lx:Icons and images":
"الأيقونات والصور",

"library.RLEX|lx:Knowing God":
"معرفة الله",

"library.RLEX|lx:Letters":
"الرسائل",

"library.RLEX|lx:Lives of the saints":
"سير القديسين",

"library.RLEX|lx:Man, soul and body":
"الإنسان، النفس والجسد",

"library.RLEX|lx:Marriage and virginity":
"الزواج والبتولية",

"library.RLEX|lx:Martyrdom and persecution":
"الاستشهاد والاضطهاد",

"library.RLEX|lx:Mathetes":
"تلميذ",

"library.RLEX|lx:Origen":
"أوريجانوس",

"library.RLEX|lx:Perpetua of Carthage and an eyewitness":
"بربتوا القرطاجية وشاهد عيان",

"library.RLEX|lx:Pontius the Deacon":
"بونتيوس الشماس",

"library.RLEX|lx:Prayer":
"الصلاة",

"library.RLEX|lx:Preaching":
"الوعظ",

"library.RLEX|lx:Reading Scripture":
"قراءة الكتاب المقدس",

"library.RLEX|lx:Repentance and confession":
"التوبة والاعتراف",

"library.RLEX|lx:Scripture opened":
"تفسير الكتاب المقدس",

"library.RLEX|lx:St Barnabas the Apostle":
"الرسول برنابا",

"library.RLEX|lx:St Basil the Great":
"القديس باسيليوس الكبير",

"library.RLEX|lx:St Clement of Rome":
"الشهيد في الكهنة كليمنضس بابا رومية",

"library.RLEX|lx:St Cyprian of Carthage":
"الشهيد في الكهنة كيبريانوس القرطاجي",

"library.RLEX|lx:St Ephraim the Syrian":
"البار أفرام السرياني",

"library.RLEX|lx:St Gregory of Nyssa":
"القديس غريغوريوس النيصصي",

"library.RLEX|lx:St Gregory the Dialogist":
"القديس غريغوريوس الكبير",

"library.RLEX|lx:St Hermas of the Seventy":
"الرسول هرماس أحد السبعين",

"library.RLEX|lx:St Hippolytus of Rome":
"الشهيد في الكهنة إيبوليتوس الروماني",

"library.RLEX|lx:St Ignatius of Antioch":
"الشهيد في الكهنة إغناطيوس الأنطاكي",

"library.RLEX|lx:St Irenaeus of Lyons":
"الشهيد في الكهنة إيريناوس أسقف ليون",

"library.RLEX|lx:St James the Brother of the Lord":
"البار يعقوب أخو الرب",

"library.RLEX|lx:St Jerome":
"القديس إيرونيموس الستريدوني",

"library.RLEX|lx:St John Cassian":
"البار يوحنا كاسيان الروماني",

"library.RLEX|lx:St John Chrysostom":
"القديس يوحنا الذهبي الفم",

"library.RLEX|lx:St John of Damascus":
"البار يوحنا الدمشقي",

"library.RLEX|lx:St Justin the Philosopher":
"الشهيد يوستينوس الفيلسوف",

"library.RLEX|lx:St Mark the Evangelist":
"الرسول والإنجيلي مرقس",

"library.RLEX|lx:St Methodius of Olympus":
"الشهيد في الكهنة ميثوديوس أسقف باتارا",

"library.RLEX|lx:St Polycarp of Smyrna":
"الشهيد في الكهنة بوليكاربوس الإزميري",

"library.RLEX|lx:Sulpitius Severus":
"سولبيكيوس ساويرس",

"library.RLEX|lx:The Christian life":
"الحياة المسيحية",

"library.RLEX|lx:The Church":
"الكنيسة",

"library.RLEX|lx:The Church of Antioch":
"كنيسة أنطاكية",

"library.RLEX|lx:The Church of Smyrna":
"كنيسة إزمير",

"library.RLEX|lx:The Councils of the Church":
"مجامع الكنيسة",

"library.RLEX|lx:The Divine Liturgy":
"القداس الإلهي",

"library.RLEX|lx:The Eucharist":
"الإفخارستيا",

"library.RLEX|lx:The Holy Spirit":
"الروح القدس",

"library.RLEX|lx:The Holy Trinity":
"الثالوث القدوس",

"library.RLEX|lx:The Roman court record":
"محاضر المحكمة الرومانية",

"library.RLEX|lx:The Theotokos":
"والدة الإله",

"library.RLEX|lx:The Twelve Apostles":
"الرسل الاثنا عشر",

"library.RLEX|lx:The archives of Edessa":
"محفوظات الرها",

"library.RLEX|lx:The councils and the canons":
"المجامع والقوانين",

"library.RLEX|lx:The departed":
"الراقدون",

"library.RLEX|lx:The history of the Church":
"تاريخ الكنيسة",

"library.RLEX|lx:The last things":
"الأمور الأخيرة",

"library.RLEX|lx:The life of prayer":
"حياة الصلاة",

"library.RLEX|lx:The monastic life":
"الحياة الرهبانية",

"library.RLEX|lx:The pastor's office":
"الخدمة الرعائية",

"library.RLEX|lx:The proconsular record":
"محاضر الوالي",

"library.RLEX|lx:The saints and their relics":
"القديسون وذخائرهم",

"library.RLEX|lx:The spiritual life":
"الحياة الروحية",

"library.RLEX|lx:Theophilus of Edessa":
"ثيوفيلوس الرهاوي",

"library.RLEX|lx:Witness to the apostles":
"الشهادة للرسل",

"library.RLEX|lx:Witnesses outside the Church":
"شهادات من خارج الكنيسة",

"library.RLEX|secBrowseDesc":
"الرف كله دفعة واحدة، مرتبا بحسب من كتبه، وبحسب القرن الذي جاء منه، وبحسب الغرض الذي كتب لأجله.",

"library.RLEX|secCouncilsDesc":
"دساتير الإيمان وقوانين المجامع المسكونية.",

"library.RLEX|secFathersDesc":
"كتابات آباء الكنيسة الأولين.",

"library.RLEX|secHome":
"الرئيسية",

"library.RLEX|secLiturgyDesc":
"الخدمة الإفخارستية: قداس القديس يوحنا الذهبي الفم بلغات متوازية، والطقوس الأقدم في أورشليم والإسكندرية.",

"library.RLEX|secLivesDesc":
"أخبار الشهداء والقديسين.",

"library.RLEX|secNTDesc":
"العهد الجديد بجميع لغات الكنيسة.",

"library.RLEX|secNTShelf":
"العهد الجديد",

"library.RLEX|secNTShelfDesc":
"العهد الجديد بلغات الكنيسة.",

"library.RLEX|secOTDesc":
"العهد القديم بحسب السبعينية، النص اليوناني الذي تسلمته الكنيسة.",

"index.I18N|ui.allLives":
"جميع سير هذا اليوم",
}
